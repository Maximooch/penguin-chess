...
class ChessGame {
    ...
    generateAlgebraicNotation(move) {
        ...
        // For captures, add 'x' and file for pawns
        if (move.captured || move.enPassant) {
            if (piece.type === 'pawn') {
                notation += from.charAt(0); // File of origin
            }
            notation += 'x';
        }
    ...
    // Get formatted move history with turn numbers
    getFormattedMoveHistory() {
        ...
        for (let i = 0; i < this.moveHistory.length; i += 2) {
            ...
            let notation = `${moveNum}. ${whiteMove.algebraic || '??'}`;
            
            if (blackMove) {
                notation += ` ${blackMove.algebraic || '??'}`;
            }
